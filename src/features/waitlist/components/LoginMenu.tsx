...
import { defaultConnectConfig } from "@/features/login/lib/config";
    ...
    <TooltipProvider>
      ...
      <div className="z-30">
        <CardHeader>
          ...
          >
            {loginTitle ?? "Login with an Arweave wallet"}
          </CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col gap-2 items-stretch">
          ...
          <Tooltip disableHoverableContent>
            <TooltipTrigger
              asChild
              onClick={async () => {
                const wallet = await connectOthentWallet(
                  defaultConnectConfig,
                  onDisconnect,
                );
                if (wallet.success) {
            ...
            <TooltipContent side="bottom">
              <p>Log in with a special wallet secured by your Google account</p>
            </TooltipContent>
          </Tooltip>
          {temporaryWalletEnabled !== false && (
            <Tooltip disableHoverableContent>
              ...
              <TooltipContent side="bottom">
                <p>Log in with a temporary wallet stored in your browser</p>
              </TooltipContent>
            </Tooltip>
          )}
        </CardContent>
